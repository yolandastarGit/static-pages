...
import { getAppPath, navigate, queryParams, withQuery } from "./router.js";
...
function userCenterPage(title, children) {
  ...
  return h("div", {}, [
    ...
    h("div", { class: "role-shell user-center-shell" }, [
      h("div", { class: "role-list" }, links.map(([label, path]) => h("a", { href: path, class: `folder-item ${getAppPath() === path ? "active" : ""}`, onclick: (event) => { event.preventDefault(); navigate(path); } }, label))),
