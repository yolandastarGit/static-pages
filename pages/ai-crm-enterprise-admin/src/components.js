...
import { canOperate, closeDrawer, closeModal, markAllNotificationsRead, markNotificationRead, openDrawer, openModal, removeNotification, state, toast } from "./store.js";
...
export function pageHeader({ title, description, breadcrumbs = [], actions = [] }) {
  return h("div", { class: "page-header" }, [
    h("div", { class: "breadcrumbs" }, breadcrumbs.map((item, index) => h("span", {}, `${index ? " / " : ""}${item}`))),
...
export function section(title, children, actions = []) {
  return h("section", { class: "card-section" }, [
    h("div", { class: "section-head" }, [h("h2", {}, title), h("div", { class: "section-actions" }, actions.filter(Boolean))]),
